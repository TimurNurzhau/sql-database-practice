...
-- =====================================================
-- ПРИМЕЧАНИЕ ПО СОРТИРОВКЕ ЗАДАЧ
-- =====================================================
-- Логика сортировки задач в GROUP_CONCAT реализована согласно
-- бизнес-правилам, описанным в задании:
--
--   * Маркетологи: порядок 10 → 13 → 2 (последовательность работ)
--   * Менеджеры и разработчики: новые задачи выше (убывание TaskID)
--   * Специалисты поддержки: старые задачи выше (возрастание TaskID)
--   * Остальные: по возрастанию TaskID
--
-- TaskID в CASE захардкожены, так как они соответствуют конкретным
-- задачам из тестовых данных. При изменении данных потребуется
-- актуализировать эти значения.