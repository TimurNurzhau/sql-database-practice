...
О проекте
В репозитории собраны решения 13 SQL-задач для четырех учебных баз данных. Каждая база данных смоделирована под реальную бизнес-сферу: транспорт, автогонки, бронирование отелей, структура организации.

Для каждой базы подготовлены:

скрипты создания таблиц (create_tables.sql)

скрипты наполнения тестовыми данными (insert_data.sql)

дампы баз данных (dump.sql) для быстрого восстановления

файлы с решениями задач (task1.sql, task2.sql и т.д.)

эталонные выводы для проверки (expected_taskN.txt)

Все запросы написаны на SQL и протестированы в MySQL через XAMPP. В репозитории настроен GitHub Actions, который при каждом пуше автоматически запускает базу данных, выполняет все запросы и сверяет результаты с эталонными файлами. Тесты проходят успешно.

В папке screenshots добавлены скриншоты результатов выполнения каждого запроса, чтобы визуально подтвердить корректность работы. Также приложен скриншот XAMPP Control Panel с запущенными службами Apache и MySQL.