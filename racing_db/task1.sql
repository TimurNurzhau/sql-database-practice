...
-- =====================================================
-- Поле year добавлено в ORDER BY как дополнительный критерий сортировки
-- для обеспечения детерминированного результата.
-- Если у двух автомобилей окажется одинаковая средняя позиция,
-- порядок между ними будет определяться годом выпуска.
-- Это не влияет на корректность, но делает вывод стабильным
